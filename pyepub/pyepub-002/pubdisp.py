...
class PubView(gtk.Window):
    ...
    def __init__(self, pvg, parent=None):
        ...
        self.stags = []; self.xtags = []; self.marks = []
    ...
    def tree_sel_row(self, xtree):
        ...
        for aa in self.stags:
            if aa == xstr:
                ...
                #self.view.scroll_to_iter(ii, 0.0, True, 0, 0)
                #self.view.scroll_to_iter(self.buffer_1.get_end_iter(), 0.0, True, 0, 0)
    ...
    def tag_iter(self, xtag, user):
        #print  xtag.get_property("size")
        #if self.xsize > 8:
        xtag.set_property("size", self.xsize * pango.SCALE)
        
        nnn = xtag.get_property("name")
        if nnn: print "xtag:", nnn
        #else:
        #    ss = view.get_default_attributes().font.get_size()
        #    self.xsize  = ss / pango.SCALE
        #    xtag.set_property("size", ss)
