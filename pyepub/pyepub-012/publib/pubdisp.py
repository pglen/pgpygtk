...
class PubView(gtk.Window):
    ...
    def __init__(self, config, parent=None):
        ...
        butt2 =       gtk.Button(" Pre_v ");  butt2.connect("clicked", self.prev)
        butt3 =       gtk.Button(" N_ext ");  butt3.connect("clicked", self.next)
        ...
        butt5a =      gtk.Button(" En_d ");   butt5a.connect("clicked", self.end)
    ...
    # This is a ping pong exip operation. We set a stop flag, and test
    # for the waiting flag to reset. If it is not reset, we issue another
    # tick until done. This aborts the file load before it kills the GUI.
    # No asserts or errors on aborted loads. Wow,
    def exit_tick(self) :
        #print "exit_tick"
        if self.waiting:
            gobject.timeout_add(300, self.exit_tick)
        else:
            self.destroy()
    
    def exit(self, butt = None):
        self.stopload = True
        # Wait for load to complete
        gobject.timeout_add(100, self.exit_tick)
    ...
    def saveconfig(self):
        ...
        if xiter:
            ...
            bcomp = self.conf.basefile + "_sel"
            #print "Saving lastsel:", bcomp, xstr
            self.conf.sql.put(bcomp, xstr)
    ...
    def sel_tree(self, xstr):
        #print "sel_tree", self.lastsel
        try:
            sel = self.treeview.get_selection()
            xmodel, xiter = sel.get_selected()
            iter = xmodel.get_iter_root()
            while  1:
                if not iter: break
                xstr2 = xmodel.get_value(iter, 0)
                if xstr == xstr2:
            ...
            pubutil.print_exception("sel_tree")
            pass
    
    
    def sel_prev(self, dir = False):
        #print "sel_prev", self.lastsel
        cnt = 0
        try:
            for aa in self.stags:
                if aa == self.lastsel and cnt > 0:
                    #print "got:",  self.xtags[cnt], self.stags[cnt]
                    #print "prev:", self.xtags[cnt-1], self.stags[cnt-1]
                    # Move tree selection
                    sel = self.treeview.get_selection()
                    xmodel, xiter = sel.get_selected()
                    iter = xmodel.get_iter_root(); old = None
                    while  1:
                        if not iter: break
                        if xmodel.get_path(iter) == xmodel.get_path(xiter):
                            break
                        old = iter
                        iter = xmodel.iter_next(iter)
                    if old:
                        sel.select_iter(old)
                        self.lastsel = xmodel.get_value(old, 0)
                    # Move in buffer
                    mm = self.buffer_1.get_mark(self.xtags[cnt-1])
                    if mm:
                        # Internal tag
                        self.view.scroll_to_mark(mm, 0.0, True, 0, 0)
                    else:
                        # This is an external link
                        self.callback(self.xtags[cnt-1])
                        self.view.grab_focus()
                        self.gohome()
                    break;
                cnt += 1
        except: 
            pubutil.print_exception("sel_prev")
            pass
            
    def sel_next(self, dir = False):
        #print "sel_next", self.lastsel
        try:
            # Move tree selection
            sel = self.treeview.get_selection()
            xmodel, xiter = sel.get_selected()
            next = xmodel.iter_next(xiter)
            if next:
                sel.select_iter(next)
                self.lastsel = xmodel.get_value(next, 0)
                self.tree_sel_row(self.treeview)
        except: 
            pubutil.print_exception("sel_next")
            pass
    ...
    def key_rel_event(self, text_view, event):
        #print "Keyrel", event, event.state, event.string, event.window, \
        #        event.state, event.group, event.hardware_keycode
        pass
    ...
    def key_press_event(self, text_view, event):
        global xsize
        #print "Keypress", event, event.state, event.string, event.window, \
        #        event.state, event.group, event.hardware_keycode
        ...
        if (event.keyval == gtk.keysyms.Return or
            ...
            if self.bscallback:
                self.bscallback()
                        ...
                        event.keyval == gtk.keysyms.equal or \
            ...
            if xsize < 100:
                ...
                event.keyval == gtk.keysyms.underscore or \
        ...
        elif event.keyval == gtk.keysyms._0 or \
                event.keyval == gtk.keysyms.KP_0 or \
                    event.keyval == gtk.keysyms.KP_Multiply: 
                bxx, byy = self.view.window_to_buffer_coords(gtk.TEXT_WINDOW_WIDGET, 0, 0)
                mm = self.view.get_iter_at_location(bxx, byy)
            ...
            if  self.hbox.flags() &  gtk.VISIBLE:
                self.hbox.hide()
            else:
                self.hbox.show()
        ...
        if event.keyval == gtk.keysyms.space:
            ...
            iter = self.buffer_1.get_iter_at_mark(self.buffer_1.get_insert())
            if iter.is_end():
                # Page to the next entry in treeview
                self.sel_next()
                self.view.grab_focus()
                self.gohome()
            else:
                # Synthesize keystroke.
                event2 = gtk.gdk.Event(gtk.gdk.KEY_PRESS)
                event2.state = event.state
                event2.window = self.view.window
                if event2.state == gtk.gdk.SHIFT_MASK:
                    event2.keyval = gtk.keysyms.Page_Up
                    event2.state = 0        # Stop selection
                    event2.hardware_keycode = 112
                else: 
                    event2.keyval = gtk.keysyms.Page_Down;
                    event2.hardware_keycode = 117
                event2.put()
