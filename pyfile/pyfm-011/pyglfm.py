...
#
# Copyright by (C) Peter Glen, 2015
#
...
from fileicon   import *
from textpad    import *
from button     import *

from py3dfont.pyfmfont import *

import pyrand as rand
...
class GLfileMan (object):
    ...
    def __init__ (self):
        ...
        self.initvars()
        self.showstars =  True 
        self.showgrid = self.showtri = False 
        ...
        self.starcol = 0.3
        self.zoffset = -50
        self.arr = [];    self.selarr = []; self.iconarr = []
        ...
        self.win.connect("scroll-event", self.scroll_event)    
    ...
    # Init icons
    def initicons(self):
        ii = FileIcon(self, "/usr/share/pixmaps/gnome-term.png", "Terminal", "gnome-terminal", 0, 0, 1)
        self.iconarr.append(ii)
        
        ii = FileIcon(self, "/usr/share/pixmaps/faces/coffee.jpg", "Coffee", "", 1, 1, 1)
        self.iconarr.append(ii)
        
        ii = FileIcon(self, "./images/calc.png", "Calulator", "gnome-calculator", -2.5, 1, 0)
        self.iconarr.append(ii)
        
        ii = FileIcon(self, "/usr/share/pixmaps/faces/sky.jpg", "Sky", "", -1, 1, 1)
        self.iconarr.append(ii)
 
        ii = FileIcon(self, "/usr/share/pixmaps/gdm-foot-logo.png", "Foot", "", -1, -1, 1)
        self.iconarr.append(ii)
 
        ii = FileIcon(self, "/usr/share/pixmaps/cupsprinter.png", "Printing", "", 1, -1, 1)
        self.iconarr.append(ii)
 
    # Init buttons
    def initbuttons(self):
        bb = Button(self, self.font8, "Press Me", "command", -3.5, -1, 0)
        self.iconarr.append(bb)
    
    # Init textpads
    def inittextpads(self):
        self.textpad = TextPad(self, self.font16, 1.5, 0, 2)
        self.iconarr.append(self.textpad)
        self.textpad.addtext("Hello World")        
        self.textpad.addtext("/pixmaps/cupsprinter.png")        
        self.textpad.addtext("Hello World is here")        
        self.textpad.addtext("/pixmaps/gdm-foot-logo.png")        
    ...
    def initfonts(self):
        ...
        self.font8.setfontcolor([ 0.929412, 0.923529, 0.927451, 1.0 ])
        self.font8.setdepth(-.05)
        
        self.font12 = GlSysFont(1./12)
        self.font12.setfontcolor([ 0.929412, 0.923529, 0.927451, 1.0 ])
        
        self.font16 = GlSysFont(1./16)
        self.font16.setdepth(.02)
        self.font16.setfontcolor([ 0.929412, 0.923529, 0.927451, 1.0 ])
    ...
    def area_button(self, area, event):
        #print "area_button", event
        if  1: #event.type == gtk.gdk.BUTTON_PRESS:
            if 1: #event.button == 1:
                ...
                for aa in res:
                    #print "Near", aa.near, "Far", aa.far, "Names", aa.names
                    lookfor = list(aa.names)
                    #self.textpad.button(aa, event)
                    for bb in self.iconarr:
                        if bb.button(aa, event):
                            break
    ...
    def area_motion(self, area, event):
        #print "motion event", event.type, event.state, event.x, event.y
        viewport = glGetIntegerv(GL_VIEWPORT)

        #self.textpad.motion(aa, event)
        for bb in self.iconarr:
            bb.motion(event)
        
        if event.state & gtk.gdk.BUTTON1_MASK:
            # Handle rotoation in the middle third, else:  shifter
            if  event.y > 2 * viewport[3] / 3 or event.y < viewport[3] / 3:
                self.__sside -=  (self.__beginX - event.x)/10.0
                self.__supdown += (self.__beginY - event.y)/10.0
            else: 
    ...
    def renderscene(self, showall = False ):
        ...
        if  0: #self.anicount < 40:
            ...
            try:
                ...
                for aa in self.iconarr:
                    aa.draw(self.pos_y, self.angle)
