...
def get_fget_func(self, strx):
    ...
    while 1:
        ...
        if len(buff) == 0:
            break
        ret = self.resp.datahandler.putdata(buff, self.resp.ekey)
        if ret == 0:
            break
...
def get_ekey_func(self, strx):
    ...
    # Do encrypt reply to ekey with old key state
...
def get_uadd_func(self, strx):
    if len(strx) < 3:
        ...
        if ret[0] == 0:
            ...
            response = "OK user already exists, no pass changed "
        elif ret[0] == 2:
...
state_table = [
            ...
            ("ekey",    auth_in,    none_in,    get_ekey_func,  ekey_help),
...
class StateHandler():
    ...
    def run_state(self, strx):
        ...
        if self.resp.ekey != "":
            strx = pycrypt.xdecrypt(strx, self.resp.ekey)
