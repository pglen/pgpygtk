...
class MainWin():
    ...
    def __init__(self, rtcflag, conf):
        ...
        try:
            ...
            try:
                window.set_icon_from_file("/usr/share/pyala/ala.png")
            except:
        ...
        self.window.connect('show', self.showme)

        if self.conf.hide:
            self.window.set_decorated(False)
            self.window.connect('destroy', self.hideme)
        else: 
            self.window.connect('destroy', self.destroyme)
