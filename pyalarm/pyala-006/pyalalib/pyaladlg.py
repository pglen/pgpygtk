...
# ------------------------------------------------------------------------
# Create a new alarm, cycle until valid or cancelled
...
def newala(strx = "", adate = ""):
    ...
    while 1:
        ...
        tt = time.mktime(ttt)
        if time.time() > tt:
            pgutil.message("Cannot set an alarm in the past.")
            continue
        exe = is_executable(parms[3])
        if not exe:
            pgutil.message("Alarm action must be an executable file.")
            continue
            
        # Save back real path
        parms2 = (parms[0], parms[1], parms[2], exe)
        break
    return parms2
...
def is_executable(fname):
    ppp = pgutil.respath(fname);  
    if ppp:
        return ppp
    reln = os.path.expanduser(fname)
    full = os.path.realpath(reln)
    if not os.access(reln, os.F_OK):
        return False
    if not os.access(reln, os.X_OK):
        return False
    return full
