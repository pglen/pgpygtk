...
def action_tick(dialog):
    ...
    '''if dialog.pobj.play:
        print "play", dialog.pobj.play
        try:
            ppp = respath("play"); 
            ret = subprocess.Popen([ppp, dialog.pobj.play])
        except:    
...
def newala(strx = ""):
    ...
    try:
        dialog.set_icon_from_file("ala2.png")
    except:
        pass
    ...
    #print dialog.pobj.__dict__
    
    add_field(dialog.pobj.date, "Date: ", dialog)
    #add_field(dialog.pobj.act,  "Act:  ", dialog)
    add_field(dialog.pobj.name, "Name: ", dialog)
    add_field(dialog.pobj.exe,  "Exe:  ", dialog)
    #add_field(dialog.pobj.arg,  "Arg:  ", dialog)
    #add_field(dialog.pobj.play, "Play: ", dialog)
    
    hbox3 = gtk.HBox()
    add_spacer(hbox3)
    dialog.vbox.pack_start(hbox3)

    # Done loading, shift name to delete status    
    fourdir  = os.path.split(strx)[0]
    try:
        if not os.path.isdir(fourdir + "/old/"):
            mkdir(fourdir + "/old/")
    except: pass
    ...
    if strx != "":
        ...
        try:
            os.rename(strx, fourdir + "/old/" + fourfile)
        except:
            #print "Cannot rename file", strx, sys.exc_info()
            pass
