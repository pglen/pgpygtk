...
class neunet():
    ...
    def __init__(self, neumap):
        ...
        for ins, neus in neumap:
            ...
            if self.curr == 0: #len(neumap)-1:
                lev = neulev(self, ins, neus, neus, trans.tfunc2)
            else:
                lev = neulev(self, ins, neus, neus, trans.tfunc2)
    ...
    def cmp(self, val):
        ...
        xlen = len(outarr)
        for aa in range(xlen): 
            #print xlen, val[aa], outarr[aa].output,
            #dd = math.sqrt(abs(sqr(val[aa]) - sqr(outarr[aa].output)))
            dd = abs(val[aa] - outarr[aa].output)
            #print dd
            diff += dd
    ...
    def trainone(self, val, passes = 100):
        ...
        self.fire(); old_sum = self.cmp(val)
