...
STATEFMT = [parser.INIT,  parser.BOLD, parser.ITALIC, \
            ...
            parser.CENT, parser.RIGHT, parser.WRAP, parser.FILL, 
            parser.INDENT, parser.SPANTXT, parser.FIXED, parser.MARGIN,\
            parser.LMARGIN ]
...
TXTCLASS = pl("ident"), pl("eq"), pl("lt"), pl("str"), pl("str2"), \
             pl("str3"), pl("gt"), pl("nl"), pl("sp"), \
                pl("tab2"), pl("tab"), pl("any"),
    

cb = pangfunc.CallBack(TextState, mainview, emit, pvg)
#cb.Text("d", "dd", "ddd")
...
parser.parsetable = \
    ...
    [ parser.INIT, None,    pl("tab"),      None,   cb.Tab,      parser.IGNORE, 0 ],       \
    [ parser.INIT, None,    pl("tab2"),     None,   cb.Tab,      parser.IGNORE, 0 ],       \
                                                                                         \
...
def showfile(strx):
    ...
    global buf, xstack, mainview, pvg
...
def help():
    ...
    print
    print myname + ":", "Utility for displaying a pango file."
    print 
...
if __name__ == "__main__":
    ...
    opts = []; args = []
    try:
        opts, args = getopt.getopt(sys.argv[1:], "d:c:hvxftopes")
