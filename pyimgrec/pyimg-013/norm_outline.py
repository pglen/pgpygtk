...
# Process the array for vector proximity
def norm_array2(fparm):
    ...
    # This will yield top lefter as element 0
    xsarr = sorted(xarr.keys())        
    ...
    curr =  xsarr[0]
    print "xsarr", curr
    for cc in range(1, wlen):
        print xsarr[cc], math.sqrt(math.pow(xsarr[cc][0] - curr[0], 2) + \
                math.pow(xsarr[cc][1] - curr[1], 2)), "  ",
    print'''
    
    # Split into two and reverse and merge
    begarr = []; endarr = [];  mid = maxy / 2     
    for dd in xsarr:
        if dd[1] <  mid:
            begarr.append(dd)
        else:
