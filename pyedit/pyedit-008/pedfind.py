...
import re, string, gtk, gconf
import peddoc, pedync
from pedutil import *
...
def find_show(self, self2):
    ...
    if self.dialog.replace:
        tit = "pyedit: Search '%s' -- Replace '%s'" % \
            (self.srctxt, self.reptxt)
    else:
        tit = "pyedit: Searching '%s'" % self.srctxt        
    ...
    #win2.connect("focus-out-event", focus_out)    
    ...
    #hbox4.pack_start(butt5, False)
    #hbox4.pack_start(butt6, False)
    #hbox4.pack_start(butt7, False)
    hbox4.pack_start(lab2)
    
    lab3 = gtk.Label("   "); lab4 = gtk.Label("   ");
    vbox = gtk.VBox()
    ...
    gconf.client_get_default().set_int(config_reg + "/srcx", oldxx)
    gconf.client_get_default().set_int(config_reg + "/srcy", oldyy)
    gconf.client_get_default().set_int(config_reg + "/srcw", oldww)
    gconf.client_get_default().set_int(config_reg + "/srch", oldhh)'''
    ...
    if self.reptxt != "":
        gconf.client_get_default().\
                set_string(config_reg + "/rep", self.reptxt)
                
    gconf.client_get_default().\
                set_int(config_reg + "/regex", self.dialog.checkbox.get_active())
                
    gconf.client_get_default().\
                set_int(config_reg + "/nocase", self.dialog.checkbox2.get_active())
