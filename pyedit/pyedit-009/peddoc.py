...
class pedDoc(gtk.DrawingArea):
    ...
    def  set_maxlinelen(self, mlen, ignore = True):
        self.maxlinelen = mlen
        self.oneshot = ignore
        self.hadj.set_all(0, 0, self.maxlinelen * 2, 1, 15, 25);
        
    def  set_maxlines(self, lines, ignore = True):              
        self.maxlines = len(self.text) + self.get_height() / self.cyy + 10
        self.oneshot = ignore
    ...
    def _draw_text(self, gc, x, y, text, foreground = None, background = None):
        ...
        if self.hex:
            ...
            for aa in text:
                tmp = "%02x " % ord(aa)
                text2 += tmp                
            self.pangolayout.set_text(text2)            
        else:
    ...
    def set_caret(self, xx, yy):
        ...
        # Needs scroll?
        xxx, yyy = self.get_size()        
        cww = xxx / self.cxx; chh = yyy / self.cyy
        need_inval = False        
        xlen = len(self.text)
        ...
        if  xx - self.xpos  > cww - self.hscgap:
            #print "Scroll from caret right", xx, self.xpos
            if self.xpos + self.caret[0] < xx:
                #print "moved r",  xx, self.caret[0], self.xpos                
                self.xpos =  xx - (cww - self.hscgap)                
                need_inval = True
        
        if  xx - self.xpos <  self.hscgap: 
            #print "Scroll from caret left ", xx, self.xpos
            if self.xpos + self.caret[0] > xx:
                #print "moved l", xx, self.caret[0], self.xpos 
        ...
        self.honeshot = True
        self.hscroll.set_value(self.xpos)        
    ...
    def loadfile(self, filename):
        self.fname = filename
        if self.fname == "": 
            print __file__, "Must specify file name."
            return False
        try:            
        ...
        mlen = self.maxlinelen
        for aa in self.text:       
            xlen = len(aa);
            if mlen < xlen: mlen = xlen
    ...
    def loadparms(self):
        ...
        self.startxxx = gconf.client_get_default().get_int\
                    (pedconfig.conf.config_reg + "/" + hhh + "/xx")
        self.startyyy = gconf.client_get_default().get_int\
                    (pedconfig.conf.config_reg + "/" + hhh + "/yy")
    ...
    def  saveparms(self):
        ...
        gconf.client_get_default().set_int\
                    (pedconfig.conf.config_reg + "/" + hhh + "/xx", 
                        self.xpos + self.caret[0])    
        gconf.client_get_default().set_int\
                    (pedconfig.conf.config_reg + "/" + hhh + "/yy", 
                        self.ypos + self.caret[1])    
        gconf.client_get_default().set_string\
                    (pedconfig.conf.config_reg + "/" + hhh + "/fname", 
                            self.fname)    
