...
class pedDoc(gtk.DrawingArea):
    ...
    def gotoxy(self, xx, yy, sel = None):
    ...
    def set_caret_middle(self, xx, yy):
        ...
        if  yy * self.cyy  > hhh:
            ...
            self.ypos = yy - (yyy / self.cyy - hhh / self.cyy ) 
            yy = yyy / self.cyy - hhh / self.cyy 
            self.invalidate()
        set_caret(xx, yy)
    ...
    def loadfile(self, filename):
        ...
        # Set up scroll bars        
        sm = len(self.text) + self.get_height() / self.cyy + 10
        self.hadj.set_all(0, 0, self.maxlinelen, 1, 15, 25);
        self.vadj.set_all(0, 0, sm, 1, 15, 25);
    ...
    # Save per file parms (cursor etc)         
    def  saveparms(self):
        hhh = hash_name(self.fname)           
