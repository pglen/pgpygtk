...
def chg_one(butt, self, self2, win2):
    sel = win2.tree.get_selection()        
    xmodel, xiter = sel.get_selected_rows()
    # In muti selection, only process first sel
    for aa in xiter:
        inspoint = xmodel.get_iter(aa)
        xstr = xmodel.get_value(inspoint, 0)    
        #print "Selected:", xstr        
        break            
    bb = xstr.split(" ")[0].split(":")
    #print "ch_one sel", bb    
    self2.gotoxy(int(bb[0]), int(bb[1]), int(bb[2]), True)
    self.ctrl_x(self2)
    self.clip_cb(None, self.reptxt, self2)
    newstr = self2.text[int(bb[1])]
    #print "newstr", newstr
                          
    # Move to next selection, execute callback
    sel.unselect_all()
    next =  win2.treestore.iter_next(inspoint)
    if next != None:
        sel.select_path(xmodel.get_path(next))            
        tree_sel_row(win2.tree, self, self2)
...
def chg_all(butt, self, self2, win2):
    ...
    root = win2.treestore.get_iter_root() 
    sel = win2.tree.get_selection()        
    xmodel, xiter = sel.get_selected_rows()
    sel.unselect_all()
    ...
    while True:
        ...
        xstr = win2.treestore.get_value(root, 0)
        root = chg_one(butt, self, self2, win2)        
        self2.mained.update_statusbar("Changing at %d" % cnt)
        usleep(1)
        cnt += 1
                        
    sel.unselect_all()
    self2.mained.update_statusbar("Changed %d items" % cnt)        
    win2.destroy()
