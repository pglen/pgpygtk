...
# 
# Pyload is released to the public domain.
...
class _Globals():
    ...
    def readconf(self):
        ...
        self.cshow = conf.sql.get("show")
        self.carm = conf.sql.get("arm")
        self.csleep = conf.sql.get("sleep")
    ...
    def writeconf(self):
        ...
        conf.sql.put("show",    self.mw.entry22.get_active())
        conf.sql.put("arm",     self.mw.entry24.get_active())
        conf.sql.put("sleep",   self.mw.entry23.get_active())
...
class MainWin(gtk.Window):
    ...
    def __init__(self, nokey = False, parent=None):
        ...
        hckbox = gtk.HBox()
        self.entry22 = gtk.CheckButton("Show Window on Actions")
        if globals.cshow != None:     
            self.entry22.set_active(int(float(globals.cshow)))
        else:
            self.entry22.set_active(True)
            
        hckbox.pack_start(text55, False);  
        hckbox.pack_start(self.entry22, False)
        hckbox.pack_start(text57, False);  
        self.entry23 = gtk.CheckButton("Enable Sleep")
        if globals.csleep != None:     
            self.entry23.set_active(int(float(globals.csleep)))
        else:
            self.entry23.set_active(True)
        hckbox.pack_start(self.entry23, False)
        hckbox.pack_start(text58, False);  
        
        self.entry24 = gtk.CheckButton("Enable Idle")
        self.entry24.connect("toggled", self.arm)
        hckbox.pack_start(self.entry24, False)
        
        hckbox.pack_start(text56, False);
        ...
        vbox2 = gtk.VBox()
        vbox2.pack_start(hckbox, False, True)
        vbox2.pack_start(buttonbox2, False, True)
        vpaned.pack2(vbox2, False, True)
...
def handler_tick():
    ...
    # Show it in impending phase
        if warn < 29 and warn > 26:
            if mw.entry22.get_active():
                mw.show()
...
def run_thread(fd, arg):
    ...
    global event_count, thdone, verbose, conf, mw
    ...
    while 1:
        try:
            ...
            if thdone:
                break
        except AttributeError, TypError:
            pass
        except:
...
def main(tray = False):
    ...
    elevate_priv()
    monitor("kbd", 0); monitor("mouse", 1)
    drop_priv()
    
    # Initial calls
    ppp = get_proc_usage();  nnn = get_net_usage()
...
def phelp():
    ...
    print "            -i          - Start hidden"
    print "            -m          - Terminate previous instance"
    print "            -v          - Verbose"
    print "            -V          - Version"
    print "            -q          - Quiet"
    print "            -h          - Help"
